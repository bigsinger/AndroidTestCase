...
static void nativeFunc_dispatcher_only_log_call(const u4 *args, jvalue *pResult, const Method *method, void *self) {
	...
	if (dvmIsStaticMethod(pOrignMethod) == false) {
		...
		dvmCallMethod_fnPtr(self, (Method *)jInvokeMethod, dvmCreateReflectMethodObject_fnPtr(pOrignMethod), &result, NULL, argArray);
	...
	if (returnType->primitiveType == PRIM_VOID) {
		...
		if (dvmUnboxPrimitive_fnPtr(result.l, returnType, pResult) == 0) {
			char msg[1024] = { 0 };
			snprintf(msg, sizeof(msg) - 1, "return error %s!=%s", ((Object *)result.l)->clazz->descriptor, returnType->descriptor);
			jni_env->ThrowNew(CastEClazz, msg);
			goto bail;
		}
	}
